...
# --- Chen (CFD) helpers: fit + predict nuisances -----------------------------

def _cfd_fit_model(Ctr, Xtr, Ztr, Ytr, seed=100, epochs=10):
    """
    Train Chen's multitask model on a training fold.
    Tries several entry points to keep compatibility with your cfd_minimal.py.
    """
    Ctr32 = Ctr.astype(np.float32, copy=False)
    XtrL  = Xtr.astype(np.int64,   copy=False)
    ZtrL  = Ztr.astype(np.int64,   copy=False)
    Ytr32 = Ytr.astype(np.float32, copy=False)

    # Preferred: a single function that returns a fitted model
    if hasattr(cfd_minimal, "train_model"):
        return cfd_minimal.train_model(Ctr32, XtrL, ZtrL, Ytr32, epochs=epochs, batch=128, lr=1e-3, h=64, seed=_seed_plus(seed, 0))

    # Otherwise: a class we can instantiate+fit
    if hasattr(cfd_minimal, "LobsterPlugIn"):
        m = cfd_minimal.LobsterPlugIn()  # allow no-arg init for your minimal file
        if hasattr(m, "fit"):
            m.fit(Ctr32, XtrL, ZtrL, Ytr32, seed=_seed_plus(seed, 0), epochs=epochs)
            return m

    raise RuntimeError("cfd_minimal must expose train_model(...) or a LobsterPlugIn with .fit(...).")

def _cfd_predict_nuisances(chen_model, C_eval, X_eval):
    """
    Return dictionary with the SAME keys as nuisance_cache_on().
    Uses model.predict_nuisances if available; otherwise computes
    from forward_* logits (torch) when present.
    """
    # 1) Direct path if your minimal module provides it:
    if hasattr(chen_model, "predict_nuisances"):
        return chen_model.predict_nuisances(C_eval, X_eval)
    if hasattr(cfd_minimal, "predict_nuisances"):
        return cfd_minimal.predict_nuisances(chen_model, C_eval, X_eval)

    # 2) Generic path via torch forward_* APIs
    try:
        import torch
        ...
        raise RuntimeError(
            "CFD: need predict_nuisances(...) or torch forward_* methods on the model."
        ) from e

    device = next(chen_model.parameters()).device if hasattr(chen_model, "parameters") else "cpu"
    Ct = torch.from_numpy(C_eval.astype(np.float32, copy=False)).to(device)
    Xt = torch.from_numpy(X_eval.astype(np.int64,   copy=False)).to(device)

    with torch.no_grad():
        # e(x|C)
        e_logits = chen_model.forward_e_logits(Ct)      # (N,2)
        e_prob   = F.softmax(e_logits, dim=1)           # (N,2)
        e1 = e_prob[:, 1].detach().cpu().numpy()

        # q(z|x,C)
        q0_logits, q1_logits = chen_model.forward_q_logits(Ct)  # (N,2) each
        q0 = F.softmax(q0_logits, dim=1)[:,1]
        q1 = F.softmax(q1_logits, dim=1)[:,1]
        q1_0c = q0.detach().cpu().numpy()
        q1_1c = q1.detach().cpu().numpy()
        q1_xc = torch.where(Xt==1, q1, q0).detach().cpu().numpy()

        # m(z,x,C)
        m00, m01, m10, m11 = chen_model.forward_m_values(Ct)
        m00 = m00.detach().cpu().numpy().reshape(-1)
        m01 = m01.detach().cpu().numpy().reshape(-1)
        m10 = m10.detach().cpu().numpy().reshape(-1)
        m11 = m11.detach().cpu().numpy().reshape(-1)

    return dict(
        e1=e1,
        q1_xc=q1_xc, q1_1c=q1_1c, q1_0c=q1_0c,
...
def nuisance_cache_on(nuis: NuisanceFold, C_eval, X_eval, delta, n_total, rng, chen_model = None):
    """
    ...
    """
        # ==== NEW: Chen (CFD) path ====
    if chen_model is not None:
        raw = _cfd_predict_nuisances(chen_model, C_eval, X_eval)

        # 1) raw numerators (keep "weight-like" probabilities raw-ish)
        e1    = cap01_raw(raw["e1"])
        q1_xc = cap01_raw(raw["q1_xc"])
        q1_1c = cap01_raw(raw["q1_1c"])
        q1_0c = cap01_raw(raw["q1_0c"])
        m11   = np.asarray(raw["m11"], dtype=float).reshape(-1)
        m01   = np.asarray(raw["m01"], dtype=float).reshape(-1)
        m10   = np.asarray(raw["m10"], dtype=float).reshape(-1)
        m00   = np.asarray(raw["m00"], dtype=float).reshape(-1)

        # 2) denominators: positivity floor (used only by FD‑DR / ratios)
        e1_den    = cap01_denom(e1)
        q1_xc_den = cap01_denom(q1_xc)

        # 3) baseline E[Y|C] for structural shrinkage of m(z,x,C)
        muC  = getattr(nuis.mC_lr, "_muC", None)
        sdC  = getattr(nuis.mC_lr, "_sdC", None)
        if muC is None or sdC is None:
            muC, sdC = _zscore_fit(C_eval)
        mC   = nuis.mC_lr.predict(_zscore_apply(C_eval, muC, sdC))

        # 4) apply δ-shrinkage with mean,var = n^{-1/4} (your default)
...
def tau_cfd_minimal_oof(C, X, Z, Y, folds, delta, bounds_y, bounds_z, seed, chen_epochs: int = 10):
    """
    Chen (CFD) plug-in τ̂(C) with *two-fold* out-of-fold training, aligned to tau_naive_oof:
        τ̂(C) = Σ_{z,x} {q̂(z|1,C)-q̂(z|0,C)} ê_x(C) m̂(z,x,C)
    Key difference vs Naive: nuisances (ê,q̂,m̂) are learned by Chen's multitask network.
    We still apply your δ-shrinkage with mean,var = n^{-1/4}.
    Returns: (tau, last_trained_model) to match call sites like “tau_r, _ = ...”.
    """
    bound_y_low, bound_y_high = bounds_y if bounds_y is not None else (float('-inf'), float('inf'))
    bound_z_low, bound_z_high = bounds_z if bounds_z is not None else (float('-inf'), float('inf'))
    n = len(X)
    tau = np.empty(n, dtype=float); tau[:] = np.nan
    last_model = None

    for k, fold in enumerate(folds):
        rng = np.random.default_rng(_seed_plus(seed, 300 + k))
        idx_tr, idx_te = fold.train_idx, fold.test_idx
        Ctr, Xtr, Ztr, Ytr = C[idx_tr], X[idx_tr], Z[idx_tr], Y[idx_tr]
        Cte, Xte = C[idx_te], X[idx_te]

        # Fit Chen model on training fold
        chen = _cfd_fit_model(Ctr, Xtr, Ztr, Ytr, seed=_seed_plus(seed, 10 + k), epochs=chen_epochs)
        last_model = chen

        # Build nuisance cache via Chen path (applies δ & n^{-1/4} noise)
        cache = nuisance_cache_on(fold.nuis, Cte, Xte, delta, n, rng, chen_model=chen)

        # Plug-in τ̂(C): identical formula to tau_naive_oof, but with Chen nuisances
        dq  = cache['q1_1c'] - cache['q1_0c']
        bound_q = list(np.array(bounds_z) - np.array(bounds_z[::-1])) if bounds_z is not None else None
        bound_q_low, bound_q_high = bound_q if bound_q is not None else (float('-inf'), float('inf'))
        dq = random_clip(dq,bound_q_low,bound_q_high)
        
        s1  = dq * (cache['m11'] - cache['m01'])
        s0  = dq * (cache['m10'] - cache['m00'])
        
        tau[idx_te] = cache['e1'] * s1 + cache['e0'] * s0
...
def run_one_n(n: int, d: int, R: int = 4, noise_coeff: float = 0.0, mode: str = "rate",
    ...
    for r in range(R):
        ...
        tau_cfd, _ = tau_cfd_minimal_oof(C, X, Z, Y, folds, delta, bound_y, bound_z, seed+50)
    ...
    return np.array(rms_naive), np.array(rms_dr), np.array(rms_r), np.array(rms_cfd)
...
def run_three_simulations(ns_list, d, R, noise_abs_for_n, noise_grid_for_fixed_n, fixed_n, mode, bound_z, bound_y):
    ...
    rows = []
    for n in ns_list:
        rn, rd, rr, rc = run_one_n(n, d, R=R, noise_coeff=0.0, mode=mode, base_seed=6060, verbose=(n==ns_list[0]), bound_z=bound_z, bound_y=bound_y)
        mN,hN = mean_ci(rn); mD,hD = mean_ci(rd); mR,hR = mean_ci(rr); mC,hC = mean_ci(rc)
        rows.append((n,mN,hN,mD,hD,mR,hR,mC,hC))
    tab_n0 = pd.DataFrame(rows, columns=["n","Naive_mean","Naive_hw","FDDR_mean","FDDR_hw","FDR_mean","FDR_hw","FDC_mean","FDC_hw"])

    # --- Sim‑2
    rows = []
    for n in ns_list:
        rn, rd, rr, rc  = run_one_n(n, d, R=R, noise_coeff=noise_abs_for_n, mode=mode, base_seed=7070, verbose=(n==ns_list[0]), bound_z=bound_z, bound_y=bound_y)
        mN,hN = mean_ci(rn); mD,hD = mean_ci(rd); mR,hR = mean_ci(rr); mC,hC = mean_ci(rc)
        rows.append((n,mN,hN,mD,hD,mR,hR,mC,hC))
    tab_nh = pd.DataFrame(rows, columns=["n","Naive_mean","Naive_hw","FDDR_mean","FDDR_hw","FDR_mean","FDR_hw","FDC_mean","FDC_hw"])

    # --- Sim‑3
    rows = []
    for coeff in noise_grid_for_fixed_n:
        rn, rd, rr, rc = run_one_n(fixed_n, d, R=R, noise_coeff=coeff, mode=mode, base_seed=8080, verbose=(coeff==noise_grid_for_fixed_n[0]), bound_z=bound_z, bound_y=bound_y)
        mN,hN = mean_ci(rn); mD,hD = mean_ci(rd); mR,hR = mean_ci(rr); mC,hC = mean_ci(rc)
        rows.append((coeff,mN,hN,mD,hD,mR,hR,mC,hC))
    tab_noise = pd.DataFrame(rows, columns=["delta","Naive_mean","Naive_hw","FDDR_mean","FDDR_hw","FDR_mean","FDR_hw","FDC_mean","FDC_hw"])
...
def run_weak_overlap_simulation(n: int, d: int, R: int, kappa_e_grid: List[float],
    ...
    for kappa_e in kappa_e_grid:
        ...
        mN,hN = mean_ci(np.array(rms_naive)); mD,hD = mean_ci(np.array(rms_dr)); mR,hR = mean_ci(np.array(rms_r)); mC,hC = mean_ci(rms_cfd)
        rows.append((kappa_e,mN,hN,mD,hD,mR,hR,mC,hC))
    tab = pd.DataFrame(rows, columns=["kappa_e","Naive_mean","Naive_hw","FDDR_mean","FDDR_hw","FDR_mean","FDR_hw", "FDC_mean", "FDC_hw"])
    return tab
